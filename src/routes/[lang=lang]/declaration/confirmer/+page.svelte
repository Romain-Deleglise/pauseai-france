...
<article>
	<UnderlinedTitle as="h1"
		>{isEn ? 'The PauseAI statement' : 'La déclaration PauseAI'}</UnderlinedTitle
	>
	...
	<section class="card" aria-live="polite">
		...
		{:else if state === 'ready' || state === 'sending'}
			...
			<button class="submit" on:click={confirm} disabled={state === 'sending'}>
				{#if state === 'sending'}
		...
		{:else}
			...
			<button class="submit" on:click={confirm}>{isEn ? 'Try again' : 'Réessayer'}</button>
		{/if}
	</section>
</article>
